...
class MessageDeleter:
    ...
    async def _validate_group_access(self) -> bool:
        ...
        try:
            ...
            # Use the same approach as the working copier module
            target_entity = None
            
            try:
                # Try direct entity lookup first (same as copier)
                target_entity = await self.client.get_entity(self.target_group_id)
                self.logger.info(f"✅ Target group found: {target_entity.title}")
            except Exception as e:
                # Try numeric ID conversion if needed (same as copier)
                if str(self.target_group_id).startswith('-100'):
                    try:
                        numeric_id = int(self.target_group_id)
                        self.logger.info(f"Trying numeric ID for target group: {numeric_id}")
                        target_entity = await self.client.get_entity(numeric_id)
                        self.logger.info(f"✅ Target group found via numeric ID: {target_entity.title}")
                        self.target_group_id = numeric_id  # Update with working variant
                    except Exception as e2:
                        self.logger.error(f"Could not get target group via numeric ID: {e2}")
                    ...
                    self.logger.error(f"Could not get target group: {e}")
                    self.logger.error("❌ Cannot access the target group.")
                    self.logger.info("💡 Try running: python3 get_group_info.py")
                    return False
            
            # Store the entity for future operations
    ...
    async def _delete_message_batch(self, message_ids: List[int]) -> int:
        ...
        try:
            ...
            # Use target_entity if available (like in copier), otherwise fall back to target_group_id
            target = self.target_entity if self.target_entity else self.target_group_id
